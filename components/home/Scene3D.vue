...
onLoop(({ elapsed }) => {
    ...
    if (isMouseTracking.value) {
        ...
        const targetRotationY = mousePosition.value.x * maxRotationY + headRotationOffsetY.value;

        const maxRotationX = Math.PI * 0.18; // Max 54 degrees up/down
        const targetRotationX = mousePosition.value.y * maxRotationX + headRotationOffsetX.value;
...
<template>
    <TresCanvas v-bind="gl" clear-color="#111" shadows alpha window-size>
        <OrbitControls v-if="store.cameraControlMode === 'orbit'" ref="orbitControlsRef" />
        <TresPerspectiveCamera
            ref="cameraRef"
            :position="[store.cameraPosition.x, store.cameraPosition.y, store.cameraPosition.z]"
            :rotation="[store.cameraRotation.x, store.cameraRotation.y, store.cameraRotation.z]"
            :fov="45"
            :aspect="1"
            :near="0.1"
            :far="1000"
        ...
        <TresGroup v-if="boundingBox && store.showWireframe" :rotation="[0, wireFrameRotationY, 0]">
            <TresMesh :position="[0, 0, 0]">
                <TresBoxGeometry :args="[boxSize.x, boxSize.y, boxSize.z]" />
                <TresMeshBasicMaterial color="#00ff00" wireframe :wireframe-linecap="'butt'" />
            </TresMesh>
        </TresGroup>
